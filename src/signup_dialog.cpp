        ...
        if (message["status"] == "error") {
            ...
            if (errors.isMember("username"))
                dialog->username_error_label->setText(
                    QString::fromStdString("Username " + message["errors"]["username"][0].asString()));
            else
                dialog->username_error_label->setText("");

            if (errors.isMember("email"))
                dialog->email_error_label->setText(
                    QString::fromStdString("Email " + message["errors"]["email"][0].asString()));
            else
                dialog->email_error_label->setText("");

            if (errors.isMember("password"))
                dialog->password_error_label->setText(
                    QString::fromStdString("Password " + message["errors"]["password"][0].asString()));
            else
                dialog->password_error_label->setText("");

            if (errors.isMember("password_confirmation"))
                dialog->confirm_password_error_label->setText(QString::fromStdString(
                    "Password confirmation " + message["errors"]["password_confirmation"][0].asString()));
            else
